...
function addRow_band(tableID, flrprefix) 
{
    ...
    element2.innerHTML="<input type='text' name='"+flrprefix+"_cus_name[]' value='' id='"+flrprefix+"_cus_name[]' style=\"width:170px;\" />";
    ...
    element4.innerHTML="<input type='text' name='"+flrprefix+"_desg[]' value='' id='"+flrprefix+"_desg[]' style=\"width:170px;\" />";
    cell4.appendChild(element4);
    
    var cell5 =row_band.insertCell(4);
    var element5 = document.createElement("span");
    element5.innerHTML="<a href=\"javascript:void(0)\" class=\"button\" onclick=\"deleteCurrentRow_band(this)\" style=\"margin-left:5px;\"> - </a>";
...
/*  Backend Free E-Book function */
function edit_manufacture(edit){       
        var sel = jQuery('#filtersource option:selected').val();
        window.location.replace("admin.php?page=ebooks.php&filter="+sel);
        }
function user_filter(edit){       
        var sel = jQuery('#filteruser option:selected').val();
        window.location.replace("admin.php?page=ebooks.php&filter="+sel);
        }        
/* End of Backend Free E-Book function */

function send_email(url){
    var url = "http://www.eatwater.co.uk/wp-content/themes/eatwater_cleaned/ajax/";
    jQuery('.sendReminder').each(function() {
        var reminderMail = [];
        if(jQuery(this).is(':checked')){
            ...
            jQuery.ajax({
                    url: url + "ebook_reminderMail.php",
                    data: {email: email, name:name, orderid:orderid },
                    success: function(result) {
                        if(result == "Success"){
                       jQuery('#emailsendReminder').css('display', 'block');
                        }
                    }
            });
