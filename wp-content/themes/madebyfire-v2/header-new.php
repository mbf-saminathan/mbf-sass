<?php  $template = get_page_template_slug( $post->ID ); ?>
    <div id="subPage" class="portfolioPage">
        ...
        ?>
        <nav class="deskMenu menuBlk">
                    <ul>
                        <?php foreach($mainMenuItems as $mainMenuItem){ 
                            
                        ?>
                        <li <?php if($post->ID==$mainMenuItem->object_id || $post->post_type==strtolower($mainMenuItem->title) || (is_tax('portfolio_categories') && $mainMenuItem->title=="Works")||is_singular('portfolio') && $mainMenuItem->title=="Works"){ echo 'class="active"'; }?>><a href="<?php echo $mainMenuItem->url; ?>" ><?php echo $mainMenuItem->title; ?></a></li>
                       <?php } ?> 
                    </ul>
            </nav>
        <?php
